...
if __name__ == "__main__":
    ...
    results = client.search("Blue lock", limit=5)
    print(f"Found {len(results)} results")
    if results:
        anime = results[0]
        print(f"Top result: {anime['titleEnglish'] or anime['titleRomaji']} ({anime['id']})")

        episodes = client.get_episodes(anime["id"])
        print(f"Episode count: {len(episodes)}")

        if episodes:
            servers = client.get_servers(anime["id"], ep_num=1)
            print(f"Sub providers: {[p['id'] for p in servers.get('subProviders', [])]}")
            best = client.get_best_source(anime["id"], ep_num=1, type_="sub")
            print(f"Best source for ep 1 (default provider): {best}")

    recent = client.get_recent(page=1)
    print(f"Recent page 1: {len(recent['results'])} entries, hasNextPage={recent['hasNextPage']}")
    import json
    print(json.dumps(recent, indent=2, ensure_ascii=False))

    schedule = client.get_schedule_for_current_week()
    print(f"This week's schedule: {len(schedule['airingSchedules'])} entries, "
          f"total={schedule['pageInfo']['total']}")